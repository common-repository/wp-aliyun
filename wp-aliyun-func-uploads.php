...
function wp_aliyun_create_uploads_zip(){
	try
	{
		$filecratetime = date("YmdHis");
		$file_prefix = 'uploads_'.$theme_name. $filecratetime;
		$zipfilename = WP_ALIYUN_ZIP_PATH . $file_prefix . '.zip';
		
		wp_aliyun_log('正在导出上传目录['.WP_ALIYUN_UPLOADS_PATH.']');
		
		$filelist = wp_aliyun_get_path_files(WP_ALIYUN_UPLOADS_PATH);
		...
		wp_aliyun_log('上传目录['.$theme_name.']备份压缩完成！');
...
function wp_aliyun_backup_uploads2oss()
{
	...
	$zipfilename = wp_aliyun_create_uploads_zip();
	$file_path = WP_ALIYUN_ZIP_PATH . $zipfilename;
	wp_aliyun_back2aliyun(WP_ALIYUN_OSS_BASE,$file_path,get_option('OSS_PREFIX').'-'.WP_ALIYUN_OSS_UPLOADS.'/'.$zipfilename);
